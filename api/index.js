...
async function getBrowserPage() {
  try {
    ...
    await p.waitForFunction(
      "document.getElementById('rain').classList.contains('loaded')",
    );
    return p;
  } catch (e) {
    console.error(e);
...
async function handler(req, res) {
  ...
  try {
    ...
    await page.waitForFunction(
      "document.getElementById('datetime').textContent.trim().length > 0",
    );
    await page.waitForFunction(
      "document.getElementById('obs').children.length > 0",
    );

    const [time, localTime] = await page.evaluate(() => {
      const time = document
        .getElementById('datetime')
        .textContent.match(/^\d+\:\d\d/i)[0];
      const localTime = new Date()
        .toLocaleTimeString('en-US', { timeZone: 'Asia/Singapore' })
        .match(/^\d+\:\d\d/i)[0];
